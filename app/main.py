...
async def company_not_approved_handler(request: Request, exc: CompanyNotApprovedException):
    # Someone whose role matches a portal route but whose company
    # isn't APPROVED yet — send them to /home, which already knows how
    # to render the pending/rejected/suspended status page.
...
async def admin_redirect_handler(request: Request, exc: AdminRedirectException):
    # Batch 8: admin hit a company-scoped page (e.g. /seller/listings)
    # with no per-admin equivalent — send them to the admin oversight
    # page that covers the same data instead of a blunt 403.
...
def dashboard_test(
    ...
    reference page. Nothing routes here for a logged-in user anymore
    (see modules/auth/routes.py's home()), but the page is left
    working and session-aware in case it's still useful as a design
    sandbox.
    """
    if user is not None:
        context = {
